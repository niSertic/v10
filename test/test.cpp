...
namespace all_tests
{
	...
	public:
		TEST_METHOD(test_01)
		{
			...
			copy(std::istream_iterator<int>(ss), std::istream_iterator<int>(), std::back_inserter(v));
...
{ return p.x > 0 && p.y > 0; });
		...
		TEST_METHOD(test_12)
		{
			...
			std::vector<int> d(atp_points.size());
			std::adjacent_difference(atp_points.begin(), atp_points.end(), d.begin());
			auto smallest_difference = std::abs(*std::min_element(d.begin() + 1, d.end(), [](int a, int b)
